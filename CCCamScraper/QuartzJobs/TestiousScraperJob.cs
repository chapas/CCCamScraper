...
namespace CCCamScraper.QuartzJobs
{
    ...
    public class TestiousScraperJob : ScraperJobBase, IJob
    {
        public TestiousScraperJob(IServiceProvider serviceProvider) : base(serviceProvider)
        {
        }

        public async Task Execute(IJobExecutionContext context) => await CheckCCCamServerstate();

        public override async Task<List<string>> ScrapeCLinesFromUrl(string urlToScrapeFrom)
        {
            urlToScrapeFrom = UrlStringReplacement(urlToScrapeFrom);
